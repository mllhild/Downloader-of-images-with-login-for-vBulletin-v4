...
namespace Download_Images_from_futanaripalace
{
    internal class Program
    {
        ...
        private static string userFP;
        private static string passwordFP;
        public static void Main(string[] args)
        {
            GetInput();
            string directoryWithIDS = @"F:\Desktop\Futanari Palace Backup\AttachmentIDs";
            Downloader(directoryWithIDS);
            Renamer("F:\\Desktop\\FutanariPalaceBackUP");
            EndMessage();
        }
        ...
        private static void GetInput()
        {
            ...
            userFP = Console.ReadLine();
            Console.Write("password: ");
            passwordFP = Console.ReadLine();
        }

        private static void Renamer(string destinationDirectory)
        {
            var files = Directory.GetFiles(destinationDirectory);
            foreach (var file in files)
            {
                var contentType = MimeTypes.GetContentType(file);
                var fileExtension = "." + contentType.Substring(contentType.IndexOf("/") + 1);
                switch (fileExtension)// what the fuck is a pjpeg????? this MimeTypes class has some stupid cases
                {
                    case ".pjpeg":
                        fileExtension = ".jpeg";
                        break;
                ...
                string newfile = file + fileExtension;
                int count = 0;
                if (File.Exists(file))
                    do
                    {
                        count++;
                        newfile = file + " (" + count.ToString() +")" + fileExtension;
        ...
        private static void Downloader(string directoryWIthIDS)
        {
            var listOfIDsWithDupes = ReadInput(directoryWIthIDS);
            var listOfIDs = listOfIDsWithDupes.Distinct().ToList();
            CheckForDirectory();
            GetCookies(); // no idea how to test this
            
            foreach (var currentID in listOfIDs)
            {
                StartDownload(currentID);
                System.Threading.Thread.Sleep(2000);
            }
        }

        private static List<int> ReadInput(string directoryWithIDs)
        {
            var filesWithIDs = Directory.GetFiles(directoryWithIDs);
            List<int> listOfIDs = new List<int>();
            foreach (var file in filesWithIDs)
            {
        ...
        {
            directory = "F:\\Desktop\\FutanariPalaceBackUP";
        ...
        {
            cookie = login("https://www.futanaripalace.com/login.php?do=login", userFP, passwordFP);
        }

        private static void StartDownload(int currentID)
        {
            currentImageID = currentID;
            Console.WriteLine("Start Downloading ImageID:" + currentImageID.ToString());
            filePath = directory + "\\" + currentImageID.ToString() +"";
            filePath = CheckIfFileExists(filePath);
            
            System.Uri uri =
                new System.Uri("https://www.futanaripalace.com/attachment.php?attachmentid=" + currentImageID.ToString()); 
            Console.WriteLine("Saving to " + filePath);
            using (WebClient wc = new WebClient())
            {
                wc.Headers.Add(HttpRequestHeader.Cookie, cookie);
                wc.Credentials = new System.Net.NetworkCredential(userFP, passwordFP);
                wc.DownloadFileAsync (uri, filePath);
                //wc.DownloadProgressChanged += new DownloadProgressChangedEventHandler(DownloadProgressCallback);
                //wc.DownloadDataCompleted += new DownloadDataCompletedEventHandler(DownloadCompletedCallback);
            }
        }
        ...
        private static string CheckIfFileExists(string filePath)
        {
            if (File.Exists(filePath))
            {
                ...
                {
                    count++;
                    tempfilePath = filePath + "(" + count.ToString() + ")";
                }
        ...
        // add filename extension once download process is complete

        private static void DownloadProgressCallback(object sender, DownloadProgressChangedEventArgs e)
        {
            //Console.WriteLine("ID: {0}    Percentage: {1}%",currentImageID.ToString(),e.ProgressPercentage.ToString());
            if(e.ProgressPercentage == 100)
            {
                //Console.WriteLine(MimeTypes.GetContentType(filePath));
                var contentType = MimeTypes.GetContentType(filePath);
                var fileExtension = "." + contentType.Substring(contentType.IndexOf("/") + 1);
                if (fileExtension == ".pjpeg") // what the fuck is a pjpeg?????
                    fileExtension = ".jpeg";
                ...
                if (File.Exists(newfilePath))
                    ...
                    {
                        count++;
                        newfilePath = filePath + " (" + count.ToString() +")" + fileExtension;
        ...
        // this for some reason never gets called
        private static void DownloadCompletedCallback(object sender, DownloadDataCompletedEventArgs e)
        {
            Console.WriteLine("Download of ImageID "+currentImageID.ToString()+" completed");
            
            // Displays the operation identifier, and the transfer progress.
            Console.WriteLine("ImageID {0}    Result: {1} Error: {2}",
        ...
        // from some code for a vBulletin 3.x version
        // https://stackoverflow.com/questions/33318643/how-can-i-login-to-a-vbulletin-php-website-with-c
        // with the updated string values from http://forum.codecall.net/topic/53896-c-vbulletin-login-function/
        ...
        private static string ScanFileForMimeType(string fileName)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(mime))
                    mime = "application/octet-stream , string.IsNullOrEmpty = true";
                return mime;
            } catch (Exception ex)
            {
                return "application/octet-stream" + ", catch in ScanFileForMimeType, Exception: \n" + ex.Message;
            }
        }
